...
class Dataset {
    private:
        ...
        std::vector<int> m_true_clusterAssignments;
        DataPoints m_dataPoints;

        std::set<int> selectRandomIndices(int numOfClusters) const;
    public:
        Dataset(int numOfPoints, int dimensions, int trueNumOfClusters, const std::vector<int>& true_clusterAssignments, const DataPoints& dataPoints, const std::string& inputFile = "")
            : m_inputFile(inputFile), m_numOfPoints(numOfPoints), m_dimensions(dimensions),
              m_trueNumOfClusters(trueNumOfClusters), m_true_clusterAssignments(true_clusterAssignments),
        ...
        std::vector<int> getTrueClusterAssignments() const { return m_true_clusterAssignments; }
